...
run_diff_abundance <- function(
  ...
  verbose = TRUE
    ...
    # 1. Strict Class Validation Check
    if (!inherits(micara_obj, "micara_input")) {
        stop("'micara_obj' must be an object of class 'micara_input'.", call. = FALSE)
    }

    disease_vec <- micara_obj$metadata$disease
    available_levels <- unique(disease_vec[!is.na(disease_vec)])

    # 2. Reference Level Validation Check
    if (!is.null(reference_level) && !reference_level %in% available_levels) {
        stop(
            "reference_level '", reference_level, "' not found in metadata$disease. Available levels: ",
