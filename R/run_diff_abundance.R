...
#' @export
  ...
  if (length(invalid) > 0) {
    stop("Invalid feature_type specified: ", paste(invalid, collapse = ", "), 
  ...
  if (length(available_types) == 0) {
    stop("None of the requested feature types (", paste(feature_type, collapse = ", "), 
  ...
  if (length(available_types) < length(feature_type) && verbose) {
    ...
    message("[Notice] Skipping requested feature type(s) not present in micara_obj: ", 
  ...
  .run_single <- function(ft) {
    ...
    if (verbose) {
      ...
      cat("Fixed formula:   ", fix_formula, "\n")
      cat("Random formula:  ", if (is.null(cur_rand_formula)) "(none)" else cur_rand_formula, "\n")
      cat("Reference level: ", reference_level, "\n")
      cat("Method / Alpha:  ", p_adj_method, " / ", alpha, "\n", sep = "")
      cat("---------------------------------------------------\n")
    }
    ...
    significant_list <- lapply(comp_levels, function(dl) {
      ...
      if (!diff_col %in% names(res)) return(NULL)
      
      sig_rows <- res[[diff_col]] & !is.na(res[[diff_col]])
      
      if (!any(sig_rows)) return(NULL)
    ...
    if (verbose) {
      cat(
        "Significant ", ft, " identified per disease comparison (",
        if (use_robust) "robust" else "standard", " flags):\n", sep = ""
      )
      if (nrow(significant) > 0) {
        print(table(significant$disease))
      } else {
        cat("  None detected at alpha =", alpha, "\n")
      }
      cat("---------------------------------------------------\n")
    }
