...
#' @export
#' @examples
#' # Set up minimal synthetic inputs
#' set.seed(123)
...
validate_inputs <- function(
  ...
  verbose = TRUE
    ...
    if (!identical(colnames(taxa), colnames(pathways)) ||
        ...
        stop(
            "Internal alignment issue: taxa, pathways and metadata ",
    ...
    if (!identical(colnames(taxa), colnames(pathways)) ||
        !identical(colnames(taxa), metadata$sample_id)) {
        stop(
            "Internal alignment issue after disease filtering.",
            call. = FALSE
        )
    }
    ...
    if (any(!valid_samples)) {
        ...
        if (verbose) {
            warning(sprintf(
                "Removing %d sample(s) with sequencing depth < %d reads: %s",
                sum(!valid_samples),
    ...
    if (verbose) {
        ...
        removed_cov_str <- if (length(removed_covariates) > 0L) {
            paste0(paste(removed_covariates, collapse = ", "), " (> ", metadata_missing_cutoff * 100, "% missing)")
        } else {
            "None"
        }
